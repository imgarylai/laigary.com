...
import { parseSortedListSearch } from "@/components/admin/list-search";
...
export const Route = createFileRoute("/admin/interview/notes/")({
  validateSearch: parseSortedListSearch,
...
function NotesPage() {
  const { t } = useI18n();
  const { items, total, pageSize } = Route.useLoaderData();

  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold tracking-tight">{t("admin.interviewNotes")}</h1>
      <NotesListClient notes={items} total={total} pageSize={pageSize} />
    </div>
  );
}
